...
def calculate_driver_remuneration(driver, competence):
    ...
    productions = Production.objects.filter(driver=driver, competence__gte=start, competence__lte=end).exclude(status=Production.CANCELLED)
    ...
    for driver in drivers:
        if total_distance:
            for truck_id, distance in distance_by_truck.items():
                result[truck_id] += driver.monthly_fixed * distance / total_distance
        elif total_trips:
            for truck_id, count in trip_by_truck.items():
                result[truck_id] += driver.monthly_fixed * Decimal(count) / Decimal(total_trips)
    return result
...
def truck_costs(company, start, end, truck_id=None):
    ...
    fixed_total = (fixed.aggregate(total=Sum("monthly_amount"))["total"] or Decimal("0")) * period_months(start, end)
    financing = Decimal("0")
    ...
    for item in company.financing_set.filter(truck_id=truck_id) if truck_id else company.financing_set.all():
        if item.truck.financial_status == item.truck.FINANCED and item.start_date and item.start_date <= end and (not item.expected_end_date or item.expected_end_date >= start):
            months = (end.year - max(start, item.start_date).year) * 12 + end.month - max(start, item.start_date).month + 1
            financing += item.monthly_payment * max(months, 1)
...
def remuneration_truck_allocation(company, start, end, driver_ids=None):
    ...
    while month <= end:
        ...
        for driver in drivers:
            ...
            trips = Trip.objects.filter(company=company, driver=driver, status=Trip.FINISHED, started_at__date__range=(max(start, month), min(end, month_end)))
            ...
            for row in by_truck:
                ...
                result[row["truck_id"]] += total * weight
...
def production_allocation(company, start, end):
    ...
    productions = Production.objects.filter(company=company, competence__range=(start, end)).exclude(status=Production.CANCELLED)
